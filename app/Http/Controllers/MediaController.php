...
class MediaController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        activity($request, "inserted", 'medias', $Id);
    ...
    {
        if ($request->ajax()) {
            $data = Blogs::latest()->orderBy('media_id', 'desc')->get();
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $actionBtn = '<a href="/admin/media/edit/' . $row->media_id . '" class="btn btn-xs btn-warning">&nbsp;<em class="icon ni ni-edit-fill"></em></a> <button class="btn btn-xs btn-danger delete_button" data-module="medias" data-id="' . $row->media_id . '" data-table="medias" data-wherefield="media_id">&nbsp;<em class="icon ni ni-trash-fill"></em></button> <button class="btn btn-xs ' . $class . ' active_inactive_button" data-id="' . $row->media_id . '" data-status="' . $row->is_active . '" data-table="medias" data-wherefield="media_id" data-module="medias">' . $str . '</button>';
                    return $actionBtn;
                })
                ->escapeColumns([])
                ->make(true);
    ...
    {
        $result = DB::table('medias')->where('media_id', $id)->first();
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasfile('image')) {
            ...
            $imageName = time() . '_' . preg_replace('/\s+/', '_', $request->file('image')->getClientOriginalName());
            $request->file('image')->storeAs("public/other_images", $imageName);
            $exist_file = DB::table('medias')->where('media_id', $request->id)->first();
            if ($exist_file) {
        ...
        DB::table('medias')->where('media_id', $request->id)->update([
            'title' => $request->title,
            'image' => $imageName,
            'video_link' => $request->video_link,
            'description' => $request->description,
        ...
        activity($request, "updated", 'medias', $request->id);

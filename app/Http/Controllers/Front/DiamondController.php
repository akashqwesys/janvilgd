...
class DiamondController extends Controller {
                    ...
                    if (isset($v['attributes'])) {
                        foreach ($v['attributes'] as $v1) {
                            if (count($v['attributes']) > 1) {
                                ...
                                } else if (in_array($v1['name'], ['Cushion', 'CU', 'Cushion Modified Brilliant', 'Cushion Brilliant'])) {
                                ...
                                } else if (in_array($v1['name'], ['Radiant', 'RD'])) {
                                    $src_img = '/assets/images/d_images/Diamond_Shapes_Radiant_b.svg';
                                } else {
                                    $src_img = '/assets/images/d_images/Diamond_Shapes_Round_Brilliant_b.svg';
                                }
    ...
    // WORKING Withour rapaport and discount column
                ...
                if (!empty($diamonds) && count($diamonds)) {
                    ...
                    if ($cat_type->category_type == config('constant.CATEGORY_TYPE_POLISH')) {
                        foreach ($diamonds as $row) {
                            ...
                            $dummeyArray['Price/CT'] = round($total / $row['expected_polish_cts'], 2);
                            $dummeyArray['Price'] = round($total, 2);
                            $dummeyArray['Cut Grade'] = $row['attributes']['CUT'] ?? null;
                            $dummeyArray['Polish'] = $row['attributes']['POLISH'] ?? null;
                            $dummeyArray['Symmetry'] = $row['attributes']['SYMMETRY'] ?? null;
                            $dummeyArray['Depth Percent'] = $row['attributes']['DEPTH PERCENT'] ?? null;
                            $dummeyArray['Table Percent'] = $row['attributes']['TABLE PERCENT'] ?? null;
                            $dummeyArray['Lab'] = $row['attributes']['LAB'] ?? null;
                            $dummeyArray['Certificate'] = $row['attributes']['CERTIFICATE'] ?? null;
                            $dummeyArray['Certificate Url'] = $row['attributes']['CERTIFICATE URL'] ?? null;
                            ...
                            $dummeyArray['Pavilion Angle'] = $row['attributes']['PAVILION ANGLE'] ?? null;
                            $dummeyArray['Growth Type'] = $row['attributes']['GROWTH TYPE'] ?? null;
                            $dummeyArray['Location']=$row['attributes']['LOCATION'] ?? null;
                            $dummeyArray['Comment']=$row['attributes']['COMMENT'] ?? null;
                            ...
                            if (count($row['image']) > 0 && file_exists( public_path('/storage/diamond_videos/' . $row['packate_no'] . '/' . $row['image'][0]) ) ) {

<?php if ($data['title'] == 'List-Labour-Charges') {
    ?>
    <script type="text/javascript">
        $(document).ready(function () {
            var table = $('#table').DataTable({
                ...
                ajax: "{{ route('labour-charges.list') }}",
                columns: [                    
                ...
                "createdRow": function (row, data, dataIndex) {                    
                    $(row).addClass('tr_'+data['labour_charge_id']);                      
                }
            });
        });
    </script>
        ...
        $(document).ready(function () {
            $(document).on('click', '.delete_button', function () {
                ...
                $.ajax({
                    ...
                    url: "{{ route('labour-charges.delete') }}",
                    data: data,
        ...
        $(document).ready(function () {
            $(document).on('click', '.active_inactive_button', function () {
                ...
                $.ajax({
                    type: "POST",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    url: "{{ route('labour-charges.status') }}",
                    data: data,
                    success: function (res) {
                        if (res.suceess) {
                            if(status==1){

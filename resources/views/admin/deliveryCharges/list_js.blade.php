<?php if ($data['title'] == 'List-Delivery-Charges') {
    ?>
    <script type="text/javascript">
        $(document).ready(function () {
            var table = $('#table').DataTable({
                ...
                ajax: "{{ route('delivery-charges.list') }}",
                columns: [
                    {data: 'index', name: 'index'},
                    {data: 'name', name: 'name'},                    
                    {data: 'transport_name', name: 'transport_name'},
                    {data: 'from_weight', name: 'from_weight'},
                    {data: 'to_weight', name: 'to_weight'},
                ...
                "createdRow": function (row, data, dataIndex) {
                    $(row).addClass('tr_' + data['delivery_charge_id']);
                }
            });
        });
    </script>
        ...
        $(document).ready(function () {
            $(document).on('click', '.delete_button', function () {
                ...
                $.ajax({
                    ...
                    url: "{{ route('delivery-charges.delete') }}",
                    data: data,
        ...
        $(document).ready(function () {
            $(document).on('click', '.active_inactive_button', function () {
                ...
                $.ajax({
                    type: "POST",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    url: "{{ route('delivery-charges.status') }}",
                    data: data,
                    success: function (res) {
                        if (res.suceess) {
                            if(status==1){

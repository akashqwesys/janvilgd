<?php if ($data['title'] == 'List-Attribute-Groups') {
    ?>
    <script type="text/javascript">
        $(document).ready(function () {
            var table = $('#table').DataTable({
                ...
                ajax: "{{ route('attribute-groups.list') }}",
                columns: [                    
                    {data: 'index', name: 'index'},
                    {data: 'name', name: 'name'},
                    {data: 'image_required', name: 'image_required'},
                ...
                "createdRow": function (row, data, dataIndex) {                    
                    $(row).addClass('tr_'+data['attribute_group_id']);                      
                }
            });
        });
    </script>
        ...
        $(document).ready(function () {
            $(document).on('click', '.delete_button', function () {
                ...
                $.ajax({
                    ...
                    url: "{{ route('attribute-groups.delete') }}",
                    data: data,
        ...
        $(document).ready(function () {
            $(document).on('click', '.active_inactive_button', function () {
                ...
                $.ajax({
                    type: "POST",
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    url: "{{ route('attribute-groups.status') }}",
                    data: data,
                    success: function (res) {
                        if (res.suceess) {
                            if(status==1){
